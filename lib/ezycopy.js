...
// Checks for text selection first, otherwise extracts full page
function extractContent() {
  ...
  if (selectionHtml) {
    ...
    const markdown = turndown.turndown(selectionHtml);
    const content = [
      `# ${document.title}`,
      '',
    ...
    return { content, isSelection: true };
  }
  ...
  if (!article) {
    ...
    return { content, isSelection: false };
  }
 ...
 * Checks for text selection first, otherwise extracts full page
 * @returns {{content: string, images: Array, title: string, subfolder: string, isSelection: boolean}}
 */
function extractContentWithImages() {
  const turndown = createTurndownService();
  const title = document.title;

  let content, images, isSelection = false;
  ...
  if (selectionHtml) {
    ...
    images = extractImagesFromHtml(selectionHtml);
    isSelection = true;
  } else {
    // No selection - extract full page with Readability
    const docClone = document.cloneNode(true);
  ...
  return { content, images, title, subfolder, isSelection };
}
